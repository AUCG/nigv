...
export const knownFileExtensions = new Set(
    [
        "narrowpeak", "broadpeak", "peaks", "bedgraph", "wig", "gff3", "gff",
        "gtf", "aneu", "fusionjuncspan", "refflat", "seg", "bed", "vcf", "bb", 
        "bigbed", "bw", "bigwig", "bam", "tdf","refgene", "genepred", "genepredext"
    ]);

export function getFormat(igv, name) {
...
export function createTrackWithConfiguration(igv, conf) {
    const type = (undefined === conf.type) ? 'unknown_type' : conf.type.toLowerCase();
    ...
    switch (type) {
        ...
        case "variant":
            return new VariantTrack(igv, conf);
            break;

        case "alignment":
            return new BAMTrack(igv, conf);
            break;
        ...
        case "sequence":
            return new SequenceTrack(igv, conf);
            break;
...
export function inferTrackTypes(igv, config) {
 ...
 * @param config
 */
export function configTrack(igv, track, config) {
    track.config = config;
    ...
    // TODO -- remove this property, not used
...
export function setTrackLabel(track, label) {
    ...
    // export function setTrackColor(track, color) {
    //
    //     track.color = color;
    //
    //     if (track.trackView) {
...
export function paintAxis(igv, ctx, pixelWidth, pixelHeight) {
        var x1,
            x2,
            y1,
            y2,
 ...
 */
export function trackPopupMenuItemList(igv, popover, viewport, genomicLocation, xOffset, yOffset) {
    var config,
        menuItems;
 ...
 * @param trackView
 */
export function trackMenuItemList(igv, popover, trackView) {
    var menuItems = [],
        all;
...
export function trackMenuItemListHelper(itemList) {
 ...
 */
export function trackMenuItem(igv, popover, trackView, menuItemLabel, dialogLabelHandler, dialogInputValue, dialogClickHandler, doAddTopBorder) {
...
export function dataRangeMenuItem(igv, popover, trackView) {
...
export function colorPickerMenuItem(igv, popover, trackView) {
    var $e,
        clickHandler;
