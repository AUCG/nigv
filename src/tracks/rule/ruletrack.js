import RulerSweeper from './rurlesweeper';
...
export class RulerTrack {
    constructor(igvInstance) {
        igv = igvInstance;
        // 挂载graphics
        igv.graphics = new graphics();
        // this.height = 50;
        this.height = 24;
        this.name = "";
        this.id = "ruler";
    ...
    createRulerSweeper(viewport, $viewport, $viewportContent, genomicState) {
        if (undefined === this.rulerSweepers) {
            this.rulerSweepers = {};
        }
        this.rulerSweepers[ genomicState.locusIndex.toString() ] = new RulerSweeper(igv, viewport, $viewport, $viewportContent, genomicState);
    }

    locusLabelWithViewport(viewport) {
    ...
    draw(options) {
        ...
        if ('all' === options.referenceFrame.chrName.toLowerCase()) {
            ...
            ts = this._findSpacing( Math.floor(options.viewportWidth * bpPerPixel) );
        ...
        function updateLocusLabelWithGenomicState(genomicState) {
            const $e = options.viewport.$viewport.find('.igv-viewport-content-ruler-div');
