...
// Chromosome ideogram
var igv;
export default class IdeoPanel {
    constructor(igvInstance, $content_header) {
        igv = igvInstance;
        $content_header.append($('<div class="igv-ideogram-left-shim"></div>'));
        this.buildPanels($content_header);
    }

    setWidth($ideogram, width) {
        var percentage;
    ...
    buildPanels($content_header) {
        this.panels = _.map(igv.browser.genomicStateList, (genomicState) => {
            ...
            this.setWidth(panel.$ideogram, viewportContainerWidth/genomicState.locusCount);
    ...
    $empty($content_header) {
        var $a = $content_header.find('.igv-ideogram-content-div');
        $a.remove();
    }

    panelWithLocusIndex(locusIndex) {
