...
export function greyScale(value) {
    var grey = clamp(value, 0, 255);
    return "rgb(" + grey + "," + grey + "," + grey + ")";
};

export function randomGrey(min, max) {
    min = clamp(min, 0, 255);
    max = clamp(max, 0, 255);
...
export function randomRGBConstantAlpha(min, max, alpha) {
...
export const nucleotideColorComponents = {
    "A": [0, 200, 0],
    "C": [0, 0, 200],
    "T": [255, 0, 0],
    "G": [209, 113, 5],
 ...
 * @returns {}
 */
export function getCompositeColor(dest, src, alpha) {
    const r = Math.floor(alpha * src[0] + (1 - alpha) * dest[0]);
    const g = Math.floor(alpha * src[1] + (1 - alpha) * dest[1]);
    const b = Math.floor(alpha * src[2] + (1 - alpha) * dest[2]);
    return "rgb(" + r + "," + g + "," + b + ")";
};
...
export function createColorString(token) {
    if (token.includes(",")) {
 ...
 * @constructor
 */
export class GradientColorScale {
    construct(scale) {
        this.scale = scale;
...
export class PaletteColorTable {
    construct(palette) {
