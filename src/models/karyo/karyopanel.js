...
import {translateMouseCoordinates, makeToggleButton} from '../../utils/utils';

const $ = jQuery;

var igv;
export default class KaryoPanel {
    constructor(igvInstance, $parent, config) {
        igv = igvInstance;
        var self = this,
            contentDiv,
            canvas,
        ...
        this.$karyoPanelToggle = makeToggleButton(igv, 'Karyotype Panel', 'Karyotype Panel', 'showKaryo', function () {
        ...
        this.canvas.onclick = function (e) {
            ...
            var mouseX = mouseCoords.x;
            var mouseY = mouseCoords.y;
            this._navigateKaryo(igv, mouseX, mouseY);
        }
    }
    ...
    _navigateKaryo(igv, mouseX, mouseY) {
        ...
        for (var i = 0; i < igv.guichromosomes.length; i++) {
            ...
            if (g.x < mouseX && g.right > mouseX && g.y < mouseY && g.bottom > mouseY) {
                ...
                // this._log("Going to position " + center);
    ...
    resize() {
        ...
        // this._log("Resize called: width=" + canvas.clientWidth + "/" + canvas.clientHeight);
    ...
    repaint() {
        ...
        if (chromosomes.length < 1) {
            // this._log("No chromosomes yet, returning");
            return;
        }
        var nrchr = 24;
        var nrrows = 1;
        ...
        if (chromosome) {
            ...
            // this._log("Could not find chromosome " + chr);
        }
        ...
        function drawImage() {
            ...
            // now add some tracks?
            // this._log("============= PROCESSING " + igv.browser.trackViews.length + " TRACKS");
            var tracknr = 0;
            for (var i = 0; i < igv.browser.trackViews.length; i++) {
                ...
                if (track.getSummary && track.loadSummary) {
                    // this._log("Found track with summary: " + track.name);
                    ...
                    source.loadSummary("chr1", 0, 1000000, function (featureList) {
                        if (featureList) {
                            //this._log("Got summary feature list, will add to karyo track")
                            nr = 0;
                            for (chr in chromosomes) {
                            ...
                            //  this._log("Track and chr "+chr+" has no summary features");
                        }
                    });
                    tracknr++;
                }
        ...
        function drawFeatures(source, featurelist, guichrom, ideogramLeft, top, bufferCtx, ideogramWidth, ideogramHeight, longestChr, tracknr) {
            ...
            if (!guichrom) {
                //this._log("no chromosome");
                return;
            }
            if (!featurelist) {
                //this._log("Found no summary features on "+guichrom );
                return;
            }
            ...
            //  this._log("drawing " + len + " feaures of chrom " + guichrom.name);
            var dx = 1;
            for (var i = 0; i < featurelist.length; i++) {
                ...
                if (source.getColor) {
                    ...
                    // this._log("got color: "+color+" for value "+value);
                }

                var starty = scale * feature.start + top;
                var endy = scale * feature.end + top;
                var dy = Math.max(0.01, endy - starty);
                //    if (i < 3) this._log("Drawing feature  " + feature.start + "-" + feature.end + " -> " + starty + ", dy=" + dy);
        ...
        function drawIdeogram(guichrom, ideogramLeft, top, chromosome, bufferCtx, ideogramWidth, ideogramHeight, longestChr) {
            ...
            if (cytobands) {
                ...
                if (len == 0) {
                    //this._log("Chr "+JSON.stringify(chromosome)+" has no length");
                    //return;
                }
                var scale = ideogramHeight / longestChr;
            ...
            // this._log("Drawing chr name "+name+" at "+(ideogramLeft + ideogramWidth / 2 - 3*name.length));
        ...
        function loadfeatures(source, chr, start, end, guichrom, bufferCtx, tracknr) {
            // this._log("=== loadfeatures of chr " + chr + ", x=" + guichrom.x);

            source.getSummary(chr, start, end, function (featureList) {
                if (featureList) {
                    len = featureList.length;
                    // this._log(" -->- loaded: chrom " + chr + " with " + len + " summary features, drawing them");
                    drawFeatures(source, featureList, guichrom, guichrom.x, guichrom.y, bufferCtx, chrwidth, chrheight, maxLen, tracknr);
                    me.repaint();
                }
                else {
                    // this._log("Track and chr "+chr+" has no summary features yet");
                }
            });
